...
class DSB(Dataset):
    ...
    def __init__(self, dataset_path):
        ...
        self.first_image_folder = os.path.join(self.data_path,
    ...
    def clear(self):
        if os.path.exists(self.directory):
            shutil.rmtree(self.directory + 'training')
            shutil.rmtree(self.directory + 'test')
            shutil.rmtree(self.directory + 'validation')
    ...
    def prepare_image(self, image_name, set_name):
        ...
        mask_files = [os.path.join(source_mask_dir, mask) for mask in os.listdir(source_mask_dir)
                      if os.path.isfile(os.path.join(source_mask_dir, mask))]
    ...
    def generator_from_names(self, image_names):
        for image_name in image_names:
            if image_name != self.broken_image:
                ...
                mask_files = [os.path.join(mask_dir, mask) for mask in os.listdir(mask_dir)
                              if os.path.isfile(os.path.join(mask_dir, mask))]
    ...
    def get_image_names(self):
        ...
        """
        return [dir for dir in os.listdir(self.data_path) if dir != self.broken_image
                and os.path.isdir(os.path.join(self.data_path, dir))]
    ...
    def compute_image_stats(self):
        ...
        for image_name in image_names:
            ...
            image = cv2.imread(image_path, 0)
            image = image/255.
            row = (image_name, np.min(image), np.max(image), np.std(image), np.mean(image),
                   image.shape[0]/1024, image.shape[1]/1024)
            rows.append(row)
